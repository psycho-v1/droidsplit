...
contract DroidSplit is PullVault {
    ...
    function finalize(uint256 id) external {
        Status st = bounties[id].status;
        if (st == Status.Open) {
            _finalizeExpired(id);
            return;
        }
        if (st == Status.Challenging) {
            _finalizeUnchallenged(id);
            return;
        }
        if (st != Status.Disputed) revert BadState();
        _finalizeDisputed(id);
    }

    function _finalizeExpired(uint256 id) internal {
        Bounty storage b = bounties[id];
    ...
    function _finalizeUnchallenged(uint256 id) internal {
        Bounty storage b = bounties[id];
    ...
    function _reopen(Bounty storage b) internal {
        b.status = Status.Open;
    ...
    function _finalizeDisputed(uint256 id) internal {
        Bounty storage b = bounties[id];
        ...
        _settleVote(id, yes > no, yes, no);
    }

    function _settleVote(uint256 id, bool claimWins, uint256 yes, uint256 no) internal {
        Bounty storage b = bounties[id];
        uint256 loser = claimWins ? no : yes;
        uint256 winner = claimWins ? yes : no;
        uint256 toPool = loser / 10;
        epochPool[currentEpoch()] += toPool;
        emit EpochFunded(currentEpoch(), toPool);
        uint256 pot = CLAIM_BOND + CHALLENGE_BOND + loser + winner - toPool;
        if (claimWins) {
            address claimant = b.claimant;
            _payClaimant(b, id, claimant);
            credit(claimant, pot);
